...
public:
  explicit inline retainment(item_handle_ptr<value_type>&& handle) noexcept
      : handle _{handle} {
  }
...
private:
  item_handle_ptr<value_type> handle_;
};
...
public:
  explicit inline reservation(item_handle_ptr<value_type>&& handle) noexcept
      : handle _{handle} {
  }
...
private:
  ...
  class item_handle_impl : public item_handle {
  public:
...
public:
  ...
  task<reservation> reserve(View view) {
...
private:
  template<viewlike View>
  task<reservation> reserve(View view, mutex_guard&& guard) {
  ...
  item_handle_ptr add_sibling() {
    using alloc_type = detail::rebind_alloc_t<allocator_type, item_handle_impl>;
  ...
  template<viewlike View>
  task<reservation> reserve_child(View view, mutex_guard&& guard) {
  ...
  task<> release(sibling_iter sibling_pos,
    ...
    *sibling_pos = std::move(value);
    ...
    if (segment_pos->active_ && segment_pos == begin(segments_) &&
        sibling_pos == begin(segment_->siblings_)) {
  ...
  inline item_handle_ptr make_handle(segment_iter segment_pos,
                                     sibling_iter sibling_pos) {
    using alloc_type = detail::rebind_alloc_t<allocator_type, item_handle_impl>;
    return std::allocate_shared<item_handle_impl>(
        alloc_type{segments_.get_allocator()}, sibling_pos, segment_pos, *this);
  }
